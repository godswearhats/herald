...
function initializeSceneryManager(){for(var a=["tower","wall","tree","corner_hill","hill"],b=0;b<a.length;b++)createSceneryButton("img/"+a[b]+".png",a[b].replace("_"," "),!0,$("#scenery-manager"));createSceneryButton("img/red_arrow.png","Player 1 Arrow",!1,$("#game"));createSceneryButton("img/blue_arrow.png","Player 2 Arrow",!1,$("#game"))}function loadRemoteScenery(){var a=$("#scenery-url").val(),b=$("#scenery-label").val();createSceneryButton(a,b)}
function createSceneryButton(a,b,c,d){$(document.createElement("img")).attr({src:a,alt:b}).load(function(){$(document.createElement("button")).appendTo(d).on("click",{src:this.src,height:this.height,width:this.width,isScenery:c},addScenery).text(this.alt)})}
function addScenery(a){var b="scenery-"+battleUnits["#scenery"].length;battleUnits["#scenery"].push(b);b=$(document.createElement("div")).addClass("scenery").attr("id",b).css({height:a.data.height+"px",width:a.data.width+"px","z-index":10,"background-image":"url("+a.data.src+")","background-repeat":"none"}).resizable({autoHide:!0,aspectRatio:a.data.isScenery});a.data.isScenery||(b.on("resize",function(a,b){b.element.children(".scenery-size").show().text(toNearestTenth(pxToInches(b.size.height))+"in.")}),
$(document.createElement("div")).text(toNearestTenth(pxToInches(a.data.height))+"in.").addClass("scenery-size").addClass("control").hide().appendTo(b));$("#battlefield").append(b);b.playable()}
