...
$(function(){$(document).on("mouseup",stopRotate);$(".add-unit").on("click",addUnit);$(".update-unit").on("click",updateUnit);$("#add-remote-scenery").on("click",loadRemoteScenery);$("#dump-button").on("click",dumpHTML);$("#slurp-button").on("click",slurpHTML);$("#start-report").on("click",toggleReportState);$(".rank-and-file").on("change",{callback:updateModelCount},modelCountHandler);$(".model-count").on("change",{callback:validateModelCount},modelCountHandler);resetUnitManagerInfo(1);resetUnitManagerInfo(2);
for(var a=["tower","wall","tree","corner_hill","hill"],b=0;b<a.length;b++)createSceneryButton("img/"+a[b]+".png",a[b].replace("_"," "),!0,"#scenery-manager");createSceneryButton("img/red_arrow.png","Player 1",!1,"#report");createSceneryButton("img/blue_arrow.png","Player 2",!1,"#report");$("#control-panel").tabs({activate:function(a,b){$(".unit").draggable("disable");var e=b.newPanel.data("player");e?($(".unit").draggable("disable"),$(".unit.player-"+e).draggable("enable")):$(".unit").draggable("enable")}})});
...
function dumpHTML(){var a=[];$(".artifact").each(function(){a.push(getDataFromElement($(this)))});$("#battlefield-out").val(JSON.stringify(a))}
...
b.isScenery=a.data("isScenery"));return b}function slurpHTML(){!1==$("#merge").prop("checked")&&$("#battlefield").html("");for(var a=jQuery.parseJSON($("#battlefield-out").val()),b=0;b<a.length;b++){var c=a[b],d;"scenery"==c.artifactType?(d="scenery-unit-"+getNextID(".scenery"),d=createScenery(d,c)):(d="player-"+c.player+"-unit-"+getNextID(".player-"+c.player),d=createUnit(c.player,d,c));updateArtifactPosition(d,c)}}
